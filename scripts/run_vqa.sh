...
# ========= 設定 =========
QA_CSV="${QA_CSV:-output/v1/qa_all.csv}"
ZIP="${ZIP:-data/cholec80/cholecseg.zip}"
OUTPUT_DIR="${OUTPUT_DIR:-output/vqa_results}"
NUM_WORKERS="${NUM_WORKERS:-8}"

if [[ -z "${OPENAI_API_KEY:-}" ]]; then
    echo "ERROR: OPENAI_API_KEY is not set."
    echo "Usage: OPENAI_API_KEY=sk-... bash scripts/run_vqa.sh"
    exit 1
fi

GPT5_EFFORTS=(minimal low medium high)
