...
QA design (v3):
  3 simple but image-required question types:
    Q1: What surgical phase is shown in this frame?        (phase-level)
    Q2: Which surgical instrument is primarily visible?    (tool-level)
    Q3: Which anatomical structure is most prominent?      (anatomy-level)

  - LLM returns correct_answer + 5 distractors (NO position assignment).
  - Python randomly shuffles all 6 options → uniform correct_option_index (0–5).
...
- For instruments, convert technical mask names to natural surgical terms:
  "grasper" → "Grasper", "l_hook_electrocautery" → "L-hook electrocautery"
...
def build_qa_prompt(caption_json: dict, phase: str) -> str:
    anatomy_present = caption_json.get("anatomy_present", [])
    tools_present = caption_json.get("tools_present", [])

    main_anatomy = anatomy_present[0] if anatomy_present else "No anatomical structure identified"
    main_tool = tools_present[0] if tools_present else "No instrument visible"

    # Phase distractors: all Cholec80 phases except the correct one
    phase_pool = [p for p in CHOLEC80_PHASES if p != phase]
    phase_pool_text = "\n".join(f"  - {p}" for p in phase_pool)

    return f"""You are creating multiple-choice questions for a surgical video VQA benchmark.
A model will be shown a laparoscopic cholecystectomy frame and must answer by visual inspection only.

[Ground truth — for distractor generation ONLY, do NOT reveal in question stems]
- Surgical phase   : {phase}
- Primary tool     : {main_tool}
- Main anatomy     : {main_anatomy}

Generate exactly 3 questions using these fixed stems:
  Q1: "What surgical phase is shown in this frame?"
  Q2: "Which surgical instrument is primarily visible in this frame?"
  Q3: "Which anatomical structure is most prominently visible in this frame?"

For each question provide:
  "correct_answer" : exact string of the ground-truth answer (given above)
  "distractors"    : list of exactly 5 plausible-but-wrong strings

Guidelines for distractors:
  Q1 — choose exactly 5 from this phase list (do NOT include "{phase}"):
{phase_pool_text}

  Q2 — use common laparoscopic instrument names
       (e.g. Scissors, Clip applier, Suction-irrigator, Bipolar forceps, Needle driver, Specimen bag)
       Do NOT include "{main_tool}".

  Q3 — use anatomy names visible in cholecystectomy
       (e.g. liver, cystic artery, hepatic duct, omentum, abdominal wall, peritoneal fat)
       Do NOT include "{main_anatomy}".
...
{{
  "qas": [
    ...
    {{
      "id": 2,
      "question": "Which surgical instrument is primarily visible in this frame?",
      "correct_answer": "{main_tool}",
      "distractors": ["...", "...", "...", "...", "..."]
    }},
    {{
      "id": 3,
      "question": "Which anatomical structure is most prominently visible in this frame?",
      "correct_answer": "{main_anatomy}",
      "distractors": ["...", "...", "...", "...", "..."]
    }}
  ]
}}"""
...
def call_qa_llm(client: OpenAI, model: str, caption_json: dict, phase: str) -> dict:
    prompt = build_qa_prompt(caption_json, phase)
    resp = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": "You are a helpful medical AI assistant."},
...
# ========= オプションシャッフル =========
def shuffle_options(
    correct_answer: str, distractors: list[str], rng: random.Random
) -> tuple[list[str], int]:
    """correct_answer + 5 distractors をシャッフルし (options, correct_index) を返す。"""
    options = [correct_answer] + list(distractors[:5])
    # LLM が5個未満を返した場合のパディング
    while len(options) < 6:
...
def format_qa_rows(row: pd.Series, qa_json: dict, seed: int) -> list[dict]:
    """QA JSON をフラットな行リストに変換。シャッフルはここで実施。"""
    rng = random.Random(seed)
    rows = []
    for qa in qa_json.get("qas", []):
        correct_answer = qa.get("correct_answer", "")
        distractors = qa.get("distractors", [])
        options, correct_idx = shuffle_options(correct_answer, distractors, rng)
        rows.append({
            "mask_path": row["mask_path"],
            "video": int(row["video"]),
            "frame": int(row["frame"]),
...
def process_row(
    ...
    qa_json = call_qa_llm(client, qa_model, cap_json, phase)
    qa_row_list = format_qa_rows(row, qa_json, seed=idx)
...
def main(args: argparse.Namespace) -> None:
    ...
    # correct_option_index の分布を確認
    df_qa = pd.DataFrame(qa_rows)
    if not df_qa.empty and "correct_option_index" in df_qa.columns:
        dist = df_qa["correct_option_index"].value_counts().sort_index()
        print(f"\ncorrect_option_index distribution:\n{dist.to_string()}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Number of parallel API call workers (default: 16)"
    )
    parser.add_argument(
